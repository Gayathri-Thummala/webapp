...
import com.crud.MyConnection;

public class ReadInv {
	...
	public ArrayList<InitialDatasetPojo> getInvoices() {
		ArrayList<InitialDatasetPojo> AllInvoices = new ArrayList<>();
		...
		try {
			String sql_query = "SELECT temp.*, business.business_name, customer.name_customer\r\n"
					+ "FROM ((temp\r\n"
					+ "LEFT JOIN business ON temp.business_code = business.business_code)\r\n"
					+ "LEFT JOIN customer ON temp.cust_number = customer.cust_number)\r\n";
			...
			while (rs.next()) {
				...
				InitialDatasetPojo invoice = new InitialDatasetPojo();
...
//			for (InitialDatasetPojo invoice: AllInvoices) {
